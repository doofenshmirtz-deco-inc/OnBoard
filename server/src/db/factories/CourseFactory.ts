...
define(Course, async (faker: typeof Faker) => { 
	...
	course.coordinators = await factory(UserGroup)({ num: faker.random.number({ min: 1, max: 3, precision: 1 })}).create();
	course.tutors = await factory(UserGroup)({ num: faker.random.number({ min: 2, max: 8, precision: 1 })}).create();
	course.students = await factory(UserGroup)({ num: faker.random.number({ min: 10, max: 30, precision: 1 })}).create();
